...
The following concrete values falsify the original (bound-free)
statement of \texttt{actor\_critic\_two\_timescale\_converges}.
Set the feature map $\psi(s, a, i) = \uparrow s$ (coercion $\mathbb{N} \to
...
and rewards $\equiv 1$. Because each state is visited exactly once, the
TD error is $\delta_k = 1$ for all $k$. The actor then accumulates
...
\texttt{h\_actor\_summable} (summability of actor increments).
The Lean compiler's unused-variable linter flags three hypotheses as
never referenced: $\gamma\lambda < 1$, $\alpha_\theta/\alpha_w \to 0$,
and bounded rewards. Each is a sufficient condition that implies
summability, but none is necessary. The corrected theorem is therefore
...
proved modulo a shared increment-summability hypothesis. This hypothesis
encodes the stochastic-approximation theorem and awaits upstream
Mathlib formalization. The Vogels-Sprekeler convergence and
